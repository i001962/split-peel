...
YOUTUBE_SCOPES = [YOUTUBE_UPLOAD_SCOPE, YOUTUBE_EDIT_SCOPE]
...
def update_video_metadata(
    ...
    *,
    ...
    if not video_id.strip():
        raise YouTubeUploadError("YouTube video id is required")
    _validate_metadata(metadata)
    if thumbnail_path:
        _validate_thumbnail_path(thumbnail_path)
    ...
    if service:
        ...
        creds = _load_youtube_credentials(credentials_path, token_path)
        response = _execute_video_update_requests(creds, body)
...
def _execute_video_update_requests(creds: Any, body: dict[str, Any]) -> dict[str, Any]:
    try:
        import requests
        from requests.adapters import HTTPAdapter
    except ImportError as error:
        raise YouTubeUploadError("YouTube metadata update requires `requests`; install `split-peel[youtube]`.") from error
    ...
    try:
        ...
        _raise_for_youtube_response(response, "updating the YouTube video metadata")
        return response.json()
    except YouTubeUploadError:
        raise
    except Exception as error:
        raise YouTubeUploadError(_network_error_message("updating the YouTube video metadata", error)) from error
